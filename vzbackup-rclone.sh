...
if [[ ${COMMAND} == 'rehydrate' ]]; then
    ...
    --drive-chunk-size=32M copy backup_cloud_drive:/$rehydrate$CMDARCHIVE $dumpdir \
...
if [[ ${COMMAND} == 'backup-end' ]]; then
    ...
    #ls $rclonedir
    rclone --config /root/.config/rclone/rclone.conf \
    --drive-chunk-size=32M copy $tarfile backup_cloud_drive:/$timepath \
    -v --stats=60s --transfers=16 --checkers=16
fi
...
if [[ ${COMMAND} == 'job-end' ||  ${COMMAND} == 'job-abort' ]]; then
    ...
    #ls $rclonedir
    rclone --config /root/.config/rclone/rclone.conf \
    --drive-chunk-size=32M move $_filename4 backup_cloud_drive:/$timepath \
    -v --stats=60s --transfers=16 --checkers=16
